...
describe("solana-swap-2025", () => {
  ...
  it("Should swap back (B to A)", async () => {
    const connection = anchor.getProvider().connection;
    const balanceUserTokenABefore = (await connection.getTokenAccountBalance(userTokenAAccount.address)).value.uiAmount * Math.pow(10, DECIMALS_MINT_A);
    const balanceUserTokenBBefore = (await connection.getTokenAccountBalance(userTokenBAccount.address)).value.uiAmount * Math.pow(10, DECIMALS_MINT_B);
    console.log("User Token A balance before reverse swap:", balanceUserTokenABefore);
    console.log("User Token B balance before reverse swap:", balanceUserTokenBBefore);

    const amountB = new anchor.BN(100 * Math.pow(10, DECIMALS_MINT_B)); // Swap 100 Token B back
    const tx = await program.methods.swap(
      amountB,
      false, // a_to_b = false
    ).accounts({
    ...
    const balanceUserTokenAAfter = (await connection.getTokenAccountBalance(userTokenAAccount.address)).value.uiAmount * Math.pow(10, DECIMALS_MINT_A);
    const balanceUserTokenBAfter = (await connection.getTokenAccountBalance(userTokenBAccount.address)).value.uiAmount * Math.pow(10, DECIMALS_MINT_B);
    console.log("User Token A balance after reverse swap:", balanceUserTokenAAfter);
    console.log("User Token B balance after reverse swap:", balanceUserTokenBAfter);

    // Calculation: amountA = (100 * 10^6) / (2.5 * 10^6) = 40
    const expectedAmountA = 40 * Math.pow(10, DECIMALS_MINT_A);
    expect(balanceUserTokenBAfter).to.equal(balanceUserTokenBBefore - amountB.toNumber(), "User should have 100 fewer Token B");
    expect(balanceUserTokenAAfter).to.equal(balanceUserTokenABefore + expectedAmountA, "User should have 40 more Token A");
  });
});
